...
let i = 2;
let reduced= newArray.slice(0,i);
let sum= reduced.reduce(getSum);
let sumArray= [];

const checkme = (input) => {
        sumArray.push(sum);
        reduced= newArray.slice(0,input+1);
        sum= getSum(reduced[reduced.length-1],sum);
        console.log(sum);
};

while (!sumArray.includes(sum)){
...
for(let i=0; i<array.length; i++){
    ...
    while(array[i].length>0){
        console.log(array[i]);
        let count=1;
        let pos= array[i].indexOf(array[i][0]);
        while(pos>=0){
            pos= array[i].indexOf(array[i][0],pos+1);
            if(pos>0){
                count++;
                console.log(pos);
               // pos= array[i].indexOf(array[i][0],pos+1);
        ...
        console.log(count);
        let reg= new RegExp(array[i][0],'g');
        array[i]= array[i].replace(reg,'');
       // console.log(pos);
        if(count===2 && !ikilendi){
            twos++;
            ikilendi=true;
        } else if(count===3 && !uclendi) {
            threes++;
            uclendi=true;
        } else if (count===1){
...
/*const addMe = (val) => {
    ...
    array= array[0].split(',');
    for(let i=0; i<array.length; i++){
        document.getElementById('result').innerText += '"' + array[i] + '"' + ',';
    }
